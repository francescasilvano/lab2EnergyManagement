function [distortion,power] = bright_contrast_sub(immagine)
    ...
    if(length(tranf(1,1,:))==3)%if it is colored we can apply modification to brightness and saturation 
        ...
        distortion=zeros(length(0.1:0.1:0.7),length(0.1:0.1:0.4));
        power=zeros(length(0.1:0.1:0.7),length(0.1:0.1:0.4));
        for k=0.1:0.1:0.7 %the bright range is 0 1
            bright=bright-k;
            x=x+1;%bright
            y=0;
            for g=0.1:0.1:0.4  %saturation range is 0 1
        ...
        power=zeros(length(20:20:180),1);
        %decrease the brightness in the gray scale
        bright=tranf;
        for k=20:20:180 %bright from 0 to 255
            bright=bright-k;
            %calculate the name of the image transformed in order to save it
            tmp=immagine;
            array=strsplit(tmp,'.');
